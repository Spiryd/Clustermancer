...
def generate_clustered_data(n, k, output_file, cluster_std=1.0, min_distance=5.0, center_range=10.0, chunk_size=100000):
    ...
    cluster_labels = np.concatenate([
        np.full(size, i, dtype=np.uint8) for i, size in enumerate(cluster_sizes)
    ])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate a CSV file with non-overlapping clustered x, y coordinates in random order.')
